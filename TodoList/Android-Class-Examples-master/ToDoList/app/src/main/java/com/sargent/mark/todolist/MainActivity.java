...
// Added category and status as parameter to method onItemClick,Added category to newInstance method call of UpdateToDoFragment
            ...
            public void onItemClick(int pos, String description, String duedate, long id, String category, String status) {
                ...
                frag.show(fm, "updateFragmentToDo");
            }
        });
    ...
    // Added parameter category  in method onItemClick
    @Override
...
//query to get all items
    ...
    private Cursor getCategoryItems(SQLiteDatabase db, String category) {
    ...
    // Added category as parameter in addToDo method to insert category in db
    private long addToDo(SQLiteDatabase db, String description, String duedate, String category) {
        ContentValues cv = new ContentValues();
        cv.put(Contract.TABLE_TODO.COLUMN_NAME_DESCRIPTION, description);
        cv.put(Contract.TABLE_TODO.COLUMN_NAME_DUE_DATE, duedate);
        cv.put(Contract.TABLE_TODO.COLUMN_NAME_TODO_CATEGORY, category);
    ...
    // Added category as parameter in updateToDo method to  udpated todolist
    ...
    // Added category as parameter in closeUpdateDialog method
    @Override
    ...
    //  updateTodoStatus method to set status done or not
    public static int updateTodoStatus(SQLiteDatabase db, long id, boolean isChecked) {
        ContentValues TodoVal = new ContentValues();
        if (isChecked) {
            TodoVal.put(Contract.TABLE_TODO.COLUMN_NAME_TODO_STATUS, "Done");
        } else {
            TodoVal.put(Contract.TABLE_TODO.COLUMN_NAME_TODO_STATUS, "");
        }

        return db.update(Contract.TABLE_TODO.TABLE_NAME, TodoVal, Contract.TABLE_TODO._ID + "=" + id, null);
    }
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        switch (id) {
            case R.id.action_all:
                ...
                return true;
            case R.id.action_l1:
                cursor = getCategoryItems(db, getString(R.string.list_1));
                adapter.swapCursor(cursor);
                return true;
            case R.id.action_l2:
                cursor = getCategoryItems(db, getString(R.string.list_2));
                adapter.swapCursor(cursor);
                return true;
            case R.id.action_l3:
                cursor = getCategoryItems(db, getString(R.string.list_3));
                adapter.swapCursor(cursor);
                return true;
            case R.id.action_l4:
                cursor = getCategoryItems(db, getString(R.string.list_4));
                adapter.swapCursor(cursor);
                return true;
            default:
